...
export const positionIncrement = (currentPos: number, length: number): number => {
    ...
    if (childElement !== undefined && childElement !== null) {
        for (let index = 0; index < childElement.length; index++) {
            if (pos === index) {
                childElement[index].style.backgroundColor = PURPLE_COLOR;
            } else {
                childElement[index].style.backgroundColor = "rgba(216, 219, 223, 1)";
            }
        }
    }
    return pos;
